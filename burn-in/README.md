...
services:
  ...
  burnin-hub:
    ...
    ports:
      - "9100:9100"
    environment:
      BURNIN_VIGIL_URL: "http://vigil-server:8080"
      BURNIN_VIGIL_AGENT_TOKEN: "your-vigil-addon-token"
...
All configuration uses environment variables with the `BURNIN_` prefix. An optional JSON config file can be loaded via `BURNIN_CONFIG_FILE`.
...
| `BURNIN_VIGIL_URL` | `http://localhost:8080` | no | Vigil server URL |
| `BURNIN_VIGIL_AGENT_TOKEN` | | no | Bearer token for Vigil registration |
| `BURNIN_VIGIL_SERVER_PUBKEY` | | no | Path to Vigil server Ed25519 public key |
...
| Layer | Mechanism |
|-------|-----------|
| **Vigil ↔ Hub** | Bearer token (`BURNIN_VIGIL_AGENT_TOKEN`) over WebSocket |
